...
import type { FdRowSeparator } from '../separator/fd-separator.js'
...
describe('fd-section', () => {
  it('renders an uppercase, letter-spaced header', async () => {
    ...
    expect(header.shadowRoot?.textContent?.trim()).toBe('Startup')
...
describe('fd-separator', () => {
  it('insets by the row inset', async () => {
    const element = await mount<FdRowSeparator>('<fd-separator></fd-separator>')
    expect(getComputedStyle(element).paddingLeft).toBe('18px')
  })

  it('can align explicitly with the icon text edge', async () => {
    const element = await mount<FdRowSeparator>(
