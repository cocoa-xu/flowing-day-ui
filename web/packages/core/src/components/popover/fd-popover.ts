...
export class FdPopover extends FdElement {
  ...
  @property({ reflect: true }) placement: FdOverlayPlacement = 'bottom'
  ...
  override updated(changed: PropertyValues<this>): void {
    ...
    if (changed.has('label')) this.#updateTrigger()
    if (
      this.open &&
      (changed.has('placement') || changed.has('minimumWidth') || changed.has('maximumWidth'))
    ) {
      this.#position()
    }
  }
